...
uint8_t MCP23017::begin(uint8_t address)
{
	_deviceAddr = address;
	return begin();
}

uint8_t MCP23017::portMode(MCP23017Port port, uint8_t directions, uint8_t pullups, uint8_t inverted)
{
	uint8_t rc = 0;
...
uint8_t MCP23017::pinMode(uint8_t pin, uint8_t mode, bool inverted)
{
...
uint8_t MCP23017::digitalWrite(uint8_t pin, uint8_t state)
{
...
uint8_t MCP23017::write(uint16_t value)
{
...
uint8_t MCP23017::readRegister(MCP23017Register reg, uint8_t& portA, uint8_t& portB)
{
	uint8_t rc = 0;
	_bus->beginTransmission(_deviceAddr);
	_bus->write(static_cast<uint8_t>(reg));
	rc = _bus->endTransmission();
...
uint8_t MCP23017::interruptMode(MCP23017InterruptMode intMode)
{
...
uint8_t MCP23017::interrupt(MCP23017Port port, uint8_t mode)
{
	uint8_t rc;
	...
	rc = writeRegister(MCP23017Register::GPINTEN_A + port, 0xFF);
	switch(mode)
	{
	case CHANGE:
		//interrupt on change
...
uint8_t MCP23017::interruptedBy(uint8_t& portA, uint8_t& portB)
{
	return readRegister(MCP23017Register::INTF_A, portA, portB);
}

uint8_t MCP23017::disableInterrupt(MCP23017Port port)
{
	return writeRegister(MCP23017Register::GPINTEN_A + port, 0x00);
}

uint8_t MCP23017::clearInterrupts()
{
	uint8_t a, b;
	return clearInterrupts(a, b);
}

uint8_t MCP23017::clearInterrupts(uint8_t& portA, uint8_t& portB)
{
